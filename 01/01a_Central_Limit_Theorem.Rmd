...
defines a function that returns a mean of x binomial samples. Since the binomial distribution can only return a 0 or 1, then `sample_mean(n=2)` can only take values 0, 0.5, or 1.
The `for` loop runs `sample_mean(n)` for each value of `Nvals` and plots a histogram of the sample means:

```{r plot-sample-means, fig.width=7}
